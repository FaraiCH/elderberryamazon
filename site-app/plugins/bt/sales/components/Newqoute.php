...
class Newqoute extends ComponentBase
{
    public $produn;
    public $priceperkg;
    public $rateupdate;
    ...
    public function loadAssets()
    {
        ...
        $this->addJs('assets/js/form.js', 'Bt.Sales','3.1.1');
    ...
    public function loadSecondsProduct(){
        #return Catalogue::where('created_at', '>=', Carbon::now()->subDays(300)->toDateTimeString())->orderBy('id','desc')->get();
        return Catalogue::all();
    }
    public function loadReasonForQuote(){
        return ReasonForQuote::all();
    }

    public function loadReceivedNonReceived(){
        return ReceivedNonReceived::orderby('id','desc')->get();
    }
...
//        $user = Auth::getUser();
        ...
        return ModelNewquote::where('created_at', '>=', Carbon::now()->subDays(100)->toDateTimeString())->active()->orderBy('id','desc')->get();
    }
            ...
            if (Input::has('product_' . $i) &&  Input::has('unitlength_' . $i) &&  Input::has('units_' . $i)) {
                if (Input::get('product_' . $i) > 0 && Input::get('unitlength_' . $i) > 0  && Input::get('units_' . $i) > 0) {
                    ...
                    if ($product->value > 0) {
                        ...
                        $unitprice = round($product->value * $view_price * $unitlength, 2);
            ...
            if (Input::has('catalogue_' . $i) && Input::has('catalogue_units_' . $i)) {
                ...
                if (Input::get('catalogue_' . $i) > 0 && Input::get('catalogue_units_' . $i) > 0) {
                    ...
                    if (Input::get('catunitprice_' . $i) > 0) {
                        ...
                        if(isset($product->bt_product_id))
                        {
                            $bt_product = $product->bt_product_id;
                        }else
                        {
                            $bt_product = null;
                        }

                        if(isset($product->priceperkg))
                        {
                            $catalogue_priceperkg = $product->priceperkg;
                        }else
                        {
                            $catalogue_priceperkg = null;
                        }
                        if(isset($product->bt_unitlength))
                        {
                            $catalogue_bt_unitlength = $product->bt_unitlength;
                        }
                        else
                        {
                            $catalogue_bt_unitlength = null;
                        }
                        if ($price > 0) {
                            $total += $price;
                            #$arr[] = array('price' =>  $this->bigmoney($price),'unitprice' =>  $this->bigmoney($unitprice), 'product' => $product , 'units' => $units, 'unitlength' => $unitlength, 'weight' => $weight,'item' => $i );
                            ...
                            $arr[] = array('producttype' => 2, 'description' => $desc, 'product_id' =>  Input::get('catalogue_' . $i), 'price' =>  $price, 'unitprice' => $unitprice, 'product' => $product, 'units' => $units, 'unitlength' => 0, 'weight' => 0, 'totalweight' => 0, 'item' => $i,  'premiumprice' => $premiumprice, 'bt_product_id' => $bt_product, 'catalogue_priceperkg' => $catalogue_priceperkg, 'catalogue_bt_unitlength' => $catalogue_bt_unitlength);
                        }
                    }
    ...
    public function onSave(){
        ...
        if (!empty($this->page['totals'])) {
            ...
            if (Input::has('client_id') && Input::get('client_id') != null) {
                $clientSave = ClientModel::find(Input::get('client_id'));
                if (!empty($clientSave)) {
            ...
            $q->quote_ref = Input::get('quote_ref');
            $q->ponumber = Input::get('quote_ref');
            ...
            if(Input::get("requestdelivery") > 0 && empty($delObj)){
                Flash::error('Please add delivery details to the quote');
                return;
            }
            if (!empty($delObj)) {
            ...
            if ($q->id > 0) {
                ...
                if(empty($obj)){
                    Flash::error("Please add an item to the quote");
                    return;
                }
                foreach ($obj as $key => $val) {
                    if ($val['producttype'] == 1) {
                        ...
                        $i->quote_id = $q->id;
                        $i->description = $val['description'];
                        $i->btproduct_id = $val['bt_product_id'];
                        $i->priceperkg = $val['catalogue_priceperkg'];
                        $i->unitlength = $val['catalogue_bt_unitlength'];
                        $i->price = $val['price'];
                        $i->unitprice = $val['unitprice'];
                        $i->units = $val['units'];
...
//        });
    ...
    public function onBusinessClient(){
        if (Input::has('client_id') && Input::get('client_id') > 0) {
            $client = ClientModel::find(Input::get('client_id'));
            if (!empty($client)) {
                $balance = $client->limit - $client->utilization;
                return[
                    '#credit_balance' => $this->renderPartial('@credit', ['company' => $balance])
                ];
            }else{
                return[
